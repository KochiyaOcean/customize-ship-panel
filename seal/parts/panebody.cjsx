...
getOpacity = (shipId) ->
  if shipId == window._ndocks[0] or shipId == window._ndocks[1] or shipId == window._ndocks[2] or shipId == window._ndocks[3]
    return {opacity: 0.4}
  else
    return {opacity: 1}

getLabelDisplay = (shipId) ->
  if shipId == window._ndocks[0] or shipId == window._ndocks[1] or shipId == window._ndocks[2] or shipId == window._ndocks[3]
    return {}
  else
    return {display: 'none'}
...
getLabelStyle = (theme)  ->
  if window.isDarkTheme
    return {'-webkit-filter': 'drop-shadow(2px 2px 5px rgba(0,0,0,1)) grayscale(1) brightness(10)'}
  else
    return {'-webkit-filter': 'drop-shadow(0px 0px 2px rgba(255,255,255,1))'}
        ...
        for shipId, j in @props.deck.api_ship
          ...
          [
            <div className="shipItem">
              <div className="shipInfo" style={getOpacity shipId}>
                <div style={display: "flex", flexDirection: "column"}>
                  ...
                  <OverlayTrigger trigger='click' placement='right' overlay={<Tooltip>Next. {ship.api_exp[1]}</Tooltip>}>
                    <OverlayTrigger trigger={['focus','hover']} placement='right' overlay={<Tooltip>Next. {ship.api_exp[1]}</Tooltip>}>
              ...
              <div className="shipStatus" style={getLabelDisplay ship.api_id}>
                <img src={join(relative(ROOT, __dirname), '..', 'assets', 'img', 'state', 'indock.png')} style={getLabelStyle window.theme}/>
              </div>
              <div className="shipHp" style={getOpacity shipId}>
                <div style={flex: 1, display: "flex"}>
                  <span style={flex: 2}>{ship.api_nowhp} / {ship.api_maxhp}</span>
                  <span style={getCondStyle ship.api_cond}>{ship.api_cond}</span>
                </div>
                <ProgressBar style={flex: 1} bsStyle={getHpStyle ship.api_nowhp / ship.api_maxhp * 100}
                           now={ship.api_nowhp / ship.api_maxhp * 100} />
              </div>
              <span className="shipFB" style={getOpacity shipId}>
                <span style={flex: 1}>
              ...
              <div className="shipSlot" style={getOpacity shipId}>
